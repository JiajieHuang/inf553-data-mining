...
join=join.map(lambda x:((int(x[1][1][0]),x[1][0]),int(x[1][1][1])))
##### reducer#####
rddSumOfRating=join.aggregateByKey((0,0), lambda a,b: (a[0] + b,    a[1] + 1),
                                       lambda a,b: (a[0] + b[0], a[1] + b[1]))
rddAvg=rddSumOfRating.map(lambda x:(x[0],x[1][0]/x[1][1])).sortBy(lambda x:x[0][0]+(x[0][1]=='M')*0.1)
output=rddAvg.collect()

fileToOpen=open(sys.argv[4],'w')
csvCursor = csv.writer(fileToOpen)
header = ['movieId','gender','ratings']
csvCursor.writerow(header)

#print output
for v in output:
    data = ['%s' % (v[0][0]), '%s' % (v[0][1]),'%s' % (v[1])]
